...
class Battery {
	public:
		Battery(adc_channel_t channel, uint16_t voltageDividerMin, uint16_t voltageDividerMax) : _channel(channel), _voltageDividerMin(voltageDividerMin), _voltageDividerMax(voltageDividerMax) {
		...
		void tick() {
			...
			if (_sampleIndex >= _sampleCount) {
				_voltage = (_sampleSum / _sampleIndex) * _voltageDividerMax / 3300;
...
//				ESP_LOGI("Battery", "ADC: %lu, Mapped: %d", _sampleSum / _sampleIndex, _voltage);
	...
	private:
		...
		uint16_t _voltageDividerMin;
		uint16_t _voltageDividerMax;
